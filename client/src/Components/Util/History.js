                        ...
                        data.map((e, i) =>
                            props.user == "bank" ? <tr className={status == "All" ? "" : status != e.status ? "hidden" : ""}>
                                <td className='border underline decoration-dotted underline-offset-4 cursor-pointer p-3' onClick={() => setPopup(i)}>
                                    {props.user == "bank" ? (data[popup] && data[popup].userId ? data[popup].userId.name : "No Name") : (data[popup] && data[popup].bankId ? data[popup].bankId.name : "No Name")}
                                </td>
                                ...
                                <td className='border p-3 cursor-pointer underline decoration-dotted underline-offset-4' onClick={() => setPopup(i)}>
                                    {e.bankId ? e.bankId.name : "No Name"}
                                </td>
                                <td className='border p-3'>
                                    <span className={(e.status == "Pending" ? "border-metal text-metal" : (e.status == "Approved" ? "border-yellowX text-yellowX " : (e.status == "Denied" ? "border-red text-red" : "border-green text-green"))) + ' border-2 px-4 py-2 rounded-xl hover:shadow-md'}>
                                        {e.status}
                                    </span>
                                </td>
                            </tr>
